...
;../_malloc.c:191: size += HEADER_SIZE; //We need a memory for header too
	...
	pshh
	pula
	sta	_malloc_size_1_8
	stx	(_malloc_size_1_8 + 1)
...
;../_malloc.c:209: if ((((unsigned int)current_header->next) -
	...
	psha
	pulh
	ldx	(_malloc_current_header_1_9 + 1)
	lda	,x
...
;../_malloc.c:211: current_header->len) >= size)
	...
	pulh
	txa
	sub	(_malloc_size_1_8 + 1)
	pshh
	pula
	sbc	_malloc_size_1_8
	bcs	00106$
